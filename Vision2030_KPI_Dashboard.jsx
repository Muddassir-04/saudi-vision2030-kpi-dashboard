    ...
    <div style={{
      ...
      <div style={{ display: 'flex', borderBottom: '1px solid #0B1F3A1a', background: '#FFFFFF' }}>
        {KPIS.map((k, i) => {
          ...
          return (
            <div key={k.key} className="kpi-tab" onClick={() => { setActiveIdx(i); setHoverPoint(null); }}
              style={{
      ...
      <div style={{ padding: '24px 32px 40px' }}>
        ...
        <Panel title={`${active.short} — Trend & Rolling 3-Year Average`}>
          <TrendChart series={series} kpi={active} hover={hoverPoint} setHover={setHoverPoint} />
          <Legend items={[
        ...
        <Panel title={`${active.short} — Year-over-Year Variance`}>
          <YoYChart series={yoySeries} kpi={active} />
        </Panel>
...
function TrendChart({ series, kpi, hover, setHover }) {
  const W = 700, H = 280;
  ...
  const rollingValues = series.map(d => d.rolling);
  const allVals = [...values, ...rollingValues, kpi.target];
  ...
  const breakIdx = kpi.breakYear ? series.findIndex(d => d.year === kpi.breakYear) : -1;
  ...
  return (
    <div style={{ position: 'relative' }}>
      <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height: 280, display: 'block' }}>
        ...
        {tickVals.map((tv, i) => (
          <g key={i}>
            <line x1={padL} x2={W - padR} y1={y(tv)} y2={y(tv)} stroke="#0B1F3A12" strokeDasharray="3 3" />
            <text x={padL - 8} y={y(tv)} textAnchor="end" dominantBaseline="middle" fontSize="10" fontFamily="JetBrains Mono" fill="#5A6472">
              {tv.toFixed(0)}
            </text>
          </g>
        ))}
        {/* x axis labels */}
        {series.map((d, i) => (
          <text key={d.year} x={x(i)} y={H - 10} textAnchor="middle" fontSize="10" fontFamily="JetBrains Mono" fill="#5A6472">
            {d.year}
          </text>
        ))}
        {/* target line */}
        <line x1={padL} x2={W - padR} y1={y(kpi.target)} y2={y(kpi.target)} stroke="#C9A24B" strokeDasharray="4 4" strokeWidth="1.5" />
        <text x={W - padR} y={y(kpi.target) - 6} textAnchor="end" fontSize="11" fontWeight="600" fill="#C9A24B">
          2030 target: {kpi.fmt(kpi.target)}
        </text>
        {/* methodology break marker */}
        {breakIdx >= 0 && (
          <g>
            <line x1={x(breakIdx)} x2={x(breakIdx)} y1={padT} y2={padT + plotH} stroke="#B23A2F" strokeDasharray="2 2" strokeWidth="1.5" />
            <text x={x(breakIdx) + 4} y={padT + 10} fontSize="10" fontWeight="600" fill="#B23A2F">methodology rebase</text>
          </g>
        )}
        {/* rolling average line */}
        <path d={linePath('rolling')} fill="none" stroke="#1B7A5A" strokeWidth="2" strokeDasharray="5 3" />
        {/* actual line */}
        <path d={linePath('value')} fill="none" stroke="#0B1F3A" strokeWidth="2.5" />
        {/* dots + hover targets */}
        {series.map((d, i) => (
          <g key={d.year}>
            <circle cx={x(i)} cy={y(d.value)} r="3" fill="#0B1F3A" />
            <circle cx={x(i)} cy={y(d.value)} r="10" fill="transparent"
              onMouseEnter={() => setHover({ ...d, cx: x(i), cy: y(d.value) })}
      ...
      {hover && (
        <div style={{
          position: 'absolute', left: `${(hover.cx / W) * 100}%`, top: Math.max(0, (hover.cy / H) * 280 - 70),
          transform: 'translateX(-50%)', background: '#0B1F3A', color: '#F7F5F0', padding: '8px 10px',
          borderRadius: 4, fontSize: 12, maxWidth: 220, pointerEvents: 'none', zIndex: 10,
        }}>
          <div className="mono" style={{ fontWeight: 700, marginBottom: 4 }}>{hover.year}: {kpi.fmt(hover.value)}</div>
          <div style={{ color: '#8FA0BC', lineHeight: 1.4, fontSize: 11 }}>{hover.notes}</div>
        </div>
      )}
    </div>
  );
...
function YoYChart({ series, kpi }) {
  const [hoverIdx, setHoverIdx] = useState(null);
  const pts = series.filter(d => d.yoy !== null);
  const W = 700, H = 200;
  const padL = 44, padR = 24, padT = 16, padB = 28;
  const plotW = W - padL - padR;
  const plotH = H - padT - padB;

  const vals = pts.map(d => d.yoy);
  const maxAbs = Math.max(Math.abs(Math.min(...vals)), Math.abs(Math.max(...vals))) * 1.2 || 1;
  const minV = -maxAbs, maxV = maxAbs;

  const barW = plotW / pts.length * 0.6;
  const x = (i) => padL + (i + 0.5) / pts.length * plotW;
  const y = (v) => padT + plotH - ((v - minV) / (maxV - minV)) * plotH;
  const zeroY = y(0);
  ...
  return (
    <div style={{ position: 'relative' }}>
      <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height: 200, display: 'block' }}>
        <line x1={padL} x2={W - padR} y1={zeroY} y2={zeroY} stroke="#0B1F3A44" />
        {pts.map((d, i) => {
          const good = kpi.direction === 'down' ? d.yoy < 0 : d.yoy > 0;
          const barTop = Math.min(zeroY, y(d.yoy));
          const barH = Math.abs(y(d.yoy) - zeroY);
          return (
            <g key={d.year}>
              <rect x={x(i) - barW / 2} y={barTop} width={barW} height={Math.max(barH, 1)}
                fill={good ? '#1B7A5A' : '#B23A2F'} rx="2"
                onMouseEnter={() => setHoverIdx(i)} onMouseLeave={() => setHoverIdx(null)}
                style={{ cursor: 'pointer' }} />
              <text x={x(i)} y={H - 8} textAnchor="middle" fontSize="10" fontFamily="JetBrains Mono" fill="#5A6472">{d.year}</text>
            </g>
          );
        })}
      </svg>
      {hoverIdx !== null && (
        <div style={{
          position: 'absolute', left: `${(x(hoverIdx) / W) * 100}%`, top: 0,
          transform: 'translateX(-50%)', background: '#0B1F3A', color: '#F7F5F0', padding: '6px 10px',
          borderRadius: 4, fontSize: 12, pointerEvents: 'none', whiteSpace: 'nowrap',
        }} className="mono">
          {pts[hoverIdx].year}: {pts[hoverIdx].yoy > 0 ? '+' : ''}{pts[hoverIdx].yoy.toFixed(1)}%
        </div>
      )}
    </div>
  );
